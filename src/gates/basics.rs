...
const SAFE_COMMANDS: &[&str] = &[
    // Display/output
    "echo", "printf", "cat", "head", "tail", "less", "more", "bat", "batcat",
    // Listing/finding
    "ls", "eza", "lsd", "tree", "find", "fd", "locate", "which", "whereis", "type",
    // Text processing (read-only)
    "grep", "rg", "ripgrep", "awk", "cut", "sort", "uniq", "wc", "tr", "tee",
    "column", "paste", "join", "comm", "diff", "cmp", "fold", "fmt", "nl",
    "rev", "tac", "expand", "unexpand", "pr", "split", "csplit",
    // File info
    "file", "stat", "du", "df", "lsof", "readlink", "realpath", "basename", "dirname",
    // Process/system info
    "ps", "top", "htop", "btop", "procs", "pgrep", "pidof", "uptime", "w", "who",
    "whoami", "id", "groups", "uname", "hostname", "hostnamectl", "date", "cal",
    "free", "vmstat", "iostat", "nproc", "lscpu", "lsmem", "lsblk", "lspci", "lsusb",
    ...
    "tokei", "cloc", "scc", "loc",
    "jq", "yq", "gron", "fx", // JSON/YAML viewers
    "hexdump", "xxd", "od", "hexyl", "strings",
    // Help/docs
    "man", "info", "help", "tldr", "tealdeer", "cheat",
    // Misc safe
    "true", "false", "yes", "seq", "expr", "bc", "dc", "factor",
    "sleep", "wait", "time", "timeout",
...
pub fn check_basics(cmd: &CommandInfo) -> GateResult {
    ...
    if program == "sed" {
        if cmd.args.iter().any(|a| a == "-i" || a.starts_with("-i") || a.starts_with("--in-place")) {
